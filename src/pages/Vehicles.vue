<template>
  <q-page style="width: 1124px; margin: auto">
    ...
    <div class="col-12 q-pa-sm q-mt-sm">
      <q-markup-table flat bordered :search="search">
        ...
        <tbody class="bg-white">
          <tr v-for="tables in table" :key="tables.id" class="tables">
            <!-- <td>{{ users.id }}</td> -->
            <td>{{ tables.number_plate }}</td>
            <td>{{ tables.vehicle_name }}</td>
            <td>{{ tables.lon }}</td>
            <td>{{ tables.lat }}</td>
            <td>{{ tables.speed }}</td>
            <td>{{ tables.engine }}</td>
      ...
      <div class="q-pa-md dense text-right bg-white" style="height:70px" bordered>
...
export default {
  ...
  methods: {
    getProducts: function () {
      ...
      axios
        .get(
          "https://devapigateway.uzzeet.com/vehicle/vehicle/realtime/list?all=true"
        )
        .then((res) => {
          this.table = res.data.result.data;
          const star = (this.current - 1) * this.prePage;
          console.log(star);
          const end = this.current * this.prePage;
          console.log(end);
    ...
    saveProduct: function () {
      ...
      form.append("number_plate", this.number_plate);
      form.append("vehicle_name", this.vehicle_name);
      form.append("lon", this.lon);
      form.append("lat", this.lat);
      form.append("speed", this.speed);
      form.append("engine", this.engine);
      form.append("head", this.head);
      console.log(form);
      axios
        .post(
